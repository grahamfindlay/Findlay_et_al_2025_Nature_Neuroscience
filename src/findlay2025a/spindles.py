...
from ecephys import hypnogram, wne, xrsig
from ecephys import plot as eplt
from findlay2025a import agg, constants, core, hypnograms, plotting
from findlay2025a.constants import Files

logger = logging.getLogger(__name__)

YASA_SPINDLE_PROPERTIES = {
    "Start": "start_time",
    "Peak": "pk_time",
    "End": "end_time",
    "Duration": "duration",
...
def read_spindles(
    ...
    sp_fname = {
        "cortical": Files.CORTICAL_SPINDLES,
        "hippocampal": Files.HIPPOCAMPAL_SPINDLES,
    }[region]
    sp_file = nb.get_experiment_subject_file(experiment, subject, sp_fname)
    sps = pd.read_parquet(sp_file)

    do_zlog = [
        "Duration",  # Still right-skewed in both CX and HC.
        "Amplitude",  # Works well in CX. Not much needed for HC.
        "RMS",  # Works well in CX. Again, not much needed for HC.
        "AbsPower",  # Not needed for CX, but no harm done. Helps with mild left-skewness in HC.
        "RelPower",  # Helpful. Still right-skewed, but works well for HC.
        "Oscillations",  # Helpful, still a bit right-skewed in both CX and HC.
    ]
    for var in do_zlog:
        sps[f"zlog_{var}"] = scipy.stats.zscore(np.log(sps[var]))

    troughs_fname = {
        "cortical": Files.CORTICAL_SPINDLE_TROUGHS,
        "hippocampal": Files.HIPPOCAMPAL_SPINDLE_TROUGHS,
    }[region]
    troughs_file = nb.get_experiment_subject_file(experiment, subject, troughs_fname)
    troughs = np.concatenate(list(np.load(troughs_file).values()))

    return sps, troughs


def aggregate_spindles(
    region: str = "cortical",
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    def _read_spindles(subject: str, experiment: str) -> pd.DataFrame:
        sps, _ = read_spindles(subject, experiment, region=region)
        return sps
    ...
    return agg.aggregate_events(_read_spindles, "Peak", keep)
...
def select_best_spindle_channel(
    ...
    if restrict_to is None:
        ...
        restricted = rpow_profile.sel(channel=rpow_profile["acronym"].isin(restrict_to))
        best_channel = restricted.channel[restricted.argmax(dim="channel")]
...
def get_spindle_properties(
    ...
    assert lf.shape == decision_function.shape, (
...
def detect_single_channel_spindles(
    ...
    mrms_thresh = get_mrms_thresholds(mrms, params["mrms_stds_threshold"], hg)
...
def plot_spindle_timehist_summary(
    ...
    delta = xr.open_dataarray(
        core.get_cortical_bandpower_file(
            sglx_subject.name, experiment, constants.Bands.DELTA
        )
    )
    light_dark_periods, light_dark_period_labels = (
        wet.rats.cnd_hgs.get_light_dark_periods(experiment, sglx_subject)
    )
    hg = (
    ...
    plotting.plot_swa_timetrace(delta, axes[0], smoothing=8)
    plotting.plot_event_rate_timetrace(spindles, axes[1], time_col="Start")
    plotting.plot_event_amplitude_timehist(
        spindles, axes[2], time_col="Start", amp_col="Amplitude"
    )
    plotting.plot_event_duration_timehist(
        spindles,
        axes[3],
        binwidth=(180, 0.2),
        time_col="Start",
...
def examine_spindle(
    ...
    fig, axes = plt.subplots(6, 1, figsize=(32, 18), sharex=True)
