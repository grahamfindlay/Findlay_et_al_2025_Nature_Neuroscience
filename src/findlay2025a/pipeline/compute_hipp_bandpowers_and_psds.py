...
Q: Final[int] = 1  # Temporal decimation factor


def do_experiment(sglx_subject: wne.sglx.SGLXSubject, experiment: str):
    # Reference hippocampal LFPs to white matter
    hipp_lfps = core.open_hippocampal_lfps(
        sglx_subject.name, experiment, drop_bad_channels=True
    )
    wm_lfps = core.open_white_matter_lfps(
        sglx_subject.name, experiment, drop_bad_channels=True
    )
    lfps = hipp_lfps - wm_lfps.mean(dim="channel")
    lfps.attrs = hipp_lfps.attrs
    ...
    for band in DO_BANDS:
        ...
        savefile = core.get_hippocampal_bandpower_file(
            sglx_subject.name, experiment, band
        )
    ...
    savefile = core.get_hippocampal_psds_file(sglx_subject.name, experiment)
